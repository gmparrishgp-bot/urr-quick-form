...
(function(){
  ...
  function injectUI(){
    ...
    const card=document.createElement('div');card.className='card';card.id='pairCard';card.innerHTML=`
      ...
      <div id="desktopPair" style="margin-top:8px">
        <div class="row"><button id="pairPhone" class="good">Pair Phone</button><button id="disconnectPhone" class="secondary hidden">Disconnect</button><span class="muted">Load WOs here, then scan this QR with the phone camera.</span></div>
  ...
  function renderPairState(text,good=false){const el=$('pairState');if(!el)return;el.textContent=text;el.style.background=good?'#dcfce7':'#e5e7eb';if($('disconnectPhone'))$('disconnectPhone').classList.toggle('hidden',!good);}
  function renderActiveArea(){const s=$('activeAreaStatus'),a=state.activeArea||'';if(s){s.textContent=a?`Scanning: ${a}`:'Choose area';s.style.background=a?'#dcfce7':'#e5e7eb';}for(const [id,val] of [['scanServiceArea','SERVICE'],['scanSalesArea','SALES']]){const b=$(id);if(b){b.className=`${a===val?'active ':'secondary '}area-choice`;}}if($('scanStatus')&&!a&&!state.scanning)$('scanStatus').textContent='Choose Service Area or Sales Area before scanning.';}
  function setActiveArea(area,notify=true){if(!['SERVICE','SALES'].includes(area))return;state.activeArea=area;renderActiveArea();if($('scanStatus'))$('scanStatus').textContent=`${area} selected. Ready to scan.`;if(notify)send({type:'scanArea',activeArea:area});}
  function renderAudit(){if(!$('auditTable'))return;const units=uniqueUnits();let on=0,off=0,not=0;for(const u of units){const s=rowStatus(u.key);if(s==='ON LOT')on++;else if(s==='OFF LOT')off++;else not++;}$('auditSummary').textContent=`${units.length} units · ${on} on · ${off} off · ${not} not seen`;if(!units.length){$('auditTable').innerHTML='<div class="muted">Load the current work-order file on this computer.</div>';return;}$('auditTable').innerHTML=`<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr><th style="text-align:left;padding:5px">Status</th><th style="text-align:left;padding:5px">Area</th><th style="text-align:left;padding:5px">Unit</th><th style="text-align:left;padding:5px">RO</th><th style="text-align:left;padding:5px">Customer</th><th style="padding:5px">Audit</th></tr></thead><tbody>${units.map(u=>{const s=rowStatus(u.key),rec=pair.records.get(u.key)||{},label=[u.year,u.make,u.model].filter(Boolean).join(' '),id=u.vin||u.stock||u.key;return `<tr style="border-top:1px solid #eee"><td style="padding:5px;font-weight:700">${esc(s)}</td><td style="padding:5px;font-weight:700">${esc(rec.area||'')}</td><td style="padding:5px">${esc(label)}<div class="muted">${esc(id)}</div></td><td style="padding:5px">${esc(u.ros.join('/'))}</td><td style="padding:5px">${esc(u.customer)}</td><td style="padding:5px;white-space:nowrap"><button class="secondary audit-on" data-key="${esc(u.key)}" style="padding:6px 8px">On Lot</button> <button class="secondary audit-off" data-key="${esc(u.key)}" style="padding:6px 8px">Off Lot</button></td></tr>`}).join('')}</tbody></table>`;document.querySelectorAll('.audit-on').forEach(b=>b.onclick=()=>setStatus(b.dataset.key,'ON LOT','manual'));document.querySelectorAll('.audit-off').forEach(b=>b.onclick=()=>setStatus(b.dataset.key,'OFF LOT','manual'));}
  function exportAudit(){const rows=[['Status','Area','VIN','Stock','ROs','Customer','Year','Make','Model','Source','Updated']];for(const u of uniqueUnits()){const r=pair.records.get(u.key)||{};rows.push([rowStatus(u.key),r.area||'',u.vin,u.stock,u.ros.join('/'),u.customer,u.year,u.make,u.model,r.source||'',r.updatedAt||'']);}const csv=rows.map(row=>row.map(v=>'"'+String(v??'').replace(/"/g,'""')+'"').join(',')).join('\r\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv'}));a.download=`lot-walk-audit-${new Date().toISOString().slice(0,10)}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);}
  ...
  function startHost(){if(typeof Peer==='undefined'){renderPairState('Pairing library unavailable');return;}disconnect();pair.role='computer';pair.sessionId=newSessionId();renderPairState('Starting…');pair.peer=new Peer(pair.sessionId,{debug:1});pair.peer.on('open',()=>{const url=new URL(location.href);url.search='';url.searchParams.set('pair',pair.sessionId);$('pairQr').innerHTML='';new QRCode($('pairQr'),{text:url.toString(),width:220,height:220,correctLevel:QRCode.CorrectLevel.M});$('pairUrl').textContent=url.toString();$('pairQrWrap').classList.remove('hidden');renderPairState('Waiting for phone');});pair.peer.on('connection',conn=>{if(pair.conn?.open){conn.close();return;}attach(conn);});pair.peer.on('error',()=>renderPairState('Pairing error'));}
  function startPhone(id){if(typeof Peer==='undefined'){renderPairState('Pairing library unavailable');return;}pair.role='phone';pair.target=id;$('desktopPair')?.classList.add('hidden');$('phonePair')?.classList.remove('hidden');$('auditCard')?.classList.add('hidden');renderPairState('Connecting…');pair.peer=new Peer(undefined,{debug:1});pair.peer.on('open',()=>attach(pair.peer.connect(id,{reliable:true})));pair.peer.on('error',()=>{renderPairState('Pairing error');scheduleReconnect();});}
  ...
  function sendLastScan(){const m=pair.lastMatch;if(pair.role!=='phone'||!m)return false;if(!state.activeArea){$('scanStatus').textContent='Choose Service Area or Sales Area before proceeding.';return false;}let sent=false;if(m.status==='MATCH'){const r=m.rows?.[0]||{};sent=send({type:'scan',area:state.activeArea,time:new Date().toISOString(),confidence:m.confidence,evidence:m.evidence||[],vin:r.vin||'',stock:r.stock||'',rows:(m.rows||[]).map(x=>({ro:x.ro,vin:x.vin,stock:x.stock,customer:x.customer}))});}pair.lastMatch=null;return sent;}
  function areaChanged(){if(pair.role==='phone')send({type:'area',service:state.service,sales:state.sales});}

  injectUI();
  $('scanServiceArea')?.addEventListener('click',()=>setActiveArea('SERVICE'));
  $('scanSalesArea')?.addEventListener('click',()=>setActiveArea('SALES'));
