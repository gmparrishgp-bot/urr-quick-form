...
(async()=>{
  ...
  await page.waitForFunction(()=>typeof window.lotWalkScanSourceV7==='function',{timeout:30000});
  async function synthetic(text,deg){return await page.evaluate(async({text,deg})=>{const c=document.createElement('canvas');c.width=900;c.height=500;const x=c.getContext('2d');x.fillStyle='white';x.fillRect(0,0,c.width,c.height);x.translate(c.width/2,c.height/2);x.rotate(deg*Math.PI/180);x.fillStyle='black';x.textAlign='center';x.textBaseline='middle';x.font='bold 110px monospace';x.fillText(text,0,0);const scan=await window.lotWalkScanSourceV7(c);return {read:scan.text,result:document.getElementById('result').innerText};},{text,deg});}
  async function scanPath(path){return await page.evaluate(async(path)=>{const img=new Image();await new Promise((res,rej)=>{img.onload=res;img.onerror=rej;img.src=path});const t0=performance.now(),scan=await window.lotWalkScanSourceV7(img,{render:false}),elapsed=Math.round(performance.now()-t0);const useful=(scan.reads||[]).filter(r=>/\d/.test(r.text||'')&&Number(r.confidence||0)>=8).sort((a,b)=>Number(b.confidence||0)-Number(a.confidence||0)).slice(0,60).map(r=>({text:r.text,confidence:Math.round(r.confidence||0),kind:r.kind,deg:r.deg,line:r.line}));return {read:scan.text,status:scan.match?.status,confidence:scan.match?.confidence,score:scan.match?.score,evidence:scan.match?.evidence,ros:(scan.match?.rows||[]).map(r=>r.ro),elapsed,attemptCount:(scan.reads||[]).length,fast:scan.fast,completion:scan.completion,segmented:scan.segmented,reconstruction:scan.reconstruction,useful};},path);}
  ...
  await browser.close();console.log('PASS v7 synthetic orientation gates + four required real July photo gates under 5s each');
