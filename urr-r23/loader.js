(async()=>{
  try{
    const baseFiles=['urr-r20/chunk00.txt?v=23b','urr-r20/chunk01.txt?v=23b','urr-r20/chunk02.txt?v=23b','urr-r20/chunk03.txt?v=23b','urr-r20/chunk04.txt?v=23b','urr-r20/chunk05.txt?v=23b','urr-r20/chunk06.txt?v=23b'];
    const [pieces,model,helpers,analyze]=await Promise.all([
      ...
      fetch('urr-r23/model_patch.txt?v=23b',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R23 model patch missing');return r.text();}),
      fetch('urr-r22/helpers_patch.txt?v=23b',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R22 helper patch missing');return r.text();}),
      fetch('urr-r23/analyze_patch.txt?v=23b',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R23 analyzer patch missing');return r.text();})
    ]);
    let src=pieces.join('\n');
    ...
    (0,eval)(src);
    for(const extra of ['urr-r21/domain.js?v=23b','urr-r21/learning-hooks.js?v=23b']){
