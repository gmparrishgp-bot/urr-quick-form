...
// Runs the repo-trained ONNX character model against already-cropped URR fields.
(function(){
  let ort=null,session=null,classes=null,readyPromise=null;
  const MODEL="training/artifacts/urr-char.onnx";
  ...
  function allowed(mode,ch){ch=String(ch||"").toUpperCase();if(mode==="parts"||mode==="hours"||mode==="total"||mode==="ro")return /^[0-9]$/.test(ch);return /^[A-Z0-9]$/.test(ch);}
  async function init(){
    if(session&&classes)return true;
    ...
    readyPromise=(async()=>{
      ...
      session=await ort.InferenceSession.create(MODEL,{executionProviders:navigator.gpu?["webgpu","wasm"]:["wasm"],graphOptimizationLevel:"all"});
      return true;
  ...
  function segments(bin){
    ...
    // Join 1px breaks inside one pen stroke.
    ...
    for(const g of joined){const width=g[1]-g[0]+1;if(width>height*1.25){
      // Split wide touching glyph groups at low-ink valleys.
      const target=Math.max(2,Math.round(width/(height*.62)));let a=g[0];for(let n=1;n<target;n++){const nominal=Math.round(g[0]+width*n/target),r=Math.max(2,Math.round(width*.10));let cut=nominal,best=Infinity;for(let x=Math.max(a+2,nominal-r);x<=Math.min(g[1]-2,nominal+r);x++)if(proj[x]<best){best=proj[x];cut=x;}out.push([a,cut-1]);a=cut;}out.push([a,g[1]]);
    }else out.push(g);}
  ...
  function glyphTensor(bin,b){
    const {ink,w,h}=bin,gw=b.x1-b.x0+1,gh=b.y1-b.y0+1,side=Math.max(gw,gh)+6,src=new Float32Array(side*side),ox=Math.floor((side-gw)/2),oy=Math.floor((side-gh)/2);
    for(let y=0;y<gh;y++)for(let x=0;x<gw;x++)if(ink[(b.y0+y)*w+(b.x0+x)])src[(oy+y)*side+(ox+x)]=1;
    const out=new Float32Array(28*28);for(let yy=4;yy<24;yy++)for(let xx=4;xx<24;xx++){const sx=Math.min(side-1,Math.floor((xx-4)*side/20)),sy=Math.min(side-1,Math.floor((yy-4)*side/20));out[yy*28+xx]=src[sy*side+sx]*2-1;}for(let i=0;i<out.length;i++)if(out[i]===0)out[i]=-1;return out;
  }
  async function readCrop(crop){
    ...
    const normalIdx=isDot.map((v,i)=>v?-1:i).filter(i=>i>=0),results=new Map();if(normalIdx.length){const data=new Float32Array(normalIdx.length*28*28);normalIdx.forEach((idx,n)=>data.set(glyphs[idx],n*28*28));const input=new ort.Tensor("float32",data,[normalIdx.length,1,28,28]);const output=await session.run({image:input}),name=session.outputNames[0],z=output[name].data,nc=classes.length;normalIdx.forEach((idx,n)=>{const row=z.slice(n*nc,(n+1)*nc),p=softmax(row);let order=[...p.keys()].sort((a,b)=>p[b]-p[a]),pick=order.find(k=>allowed(mode,classes[k]));if(pick==null)pick=order[0];results.set(idx,{ch:String(classes[pick]).toUpperCase(),conf:p[pick]});});}
    let text="",conf=[],prev=null;const heights=segs.map(b=>b.y1-b.y0+1).sort((a,b)=>a-b),mh=heights[Math.floor(heights.length/2)]||bin.h*.5,detail=[];for(let i=0;i<segs.length;i++){const b=segs[i],r=isDot[i]?{ch:".",conf:.99}:results.get(i);if(!r)continue;if(prev&&mode.startsWith("description")&&b.x0-prev.x1>mh*.55)text+=" ";text+=r.ch;conf.push(r.conf);detail.push({char:r.ch,confidence:r.conf,box:b});prev=b;}
  ...
  window.URR_CHAR_ENGINE={init,readCrop,readBatch,model:MODEL};
})();
