...
(function(){
  ...
  function tokenCorrect(tok){const low=tok.toLowerCase();if(WORDSET.has(low)||low.length<4)return tok;let best=low,bd=99;for(const w of WORDS){if(Math.abs(w.length-low.length)>2)continue;const d=ed(low,w);if(d<bd){bd=d;best=w;}}const limit=low.length>=9?2:1;return bd<=limit?best:tok;}
