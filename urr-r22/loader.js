...
(async()=>{
  try{
    ...
    const [pieces,model,helpers,analyze]=await Promise.all([
      ...
      fetch('urr-r22/model_patch.txt?v=22a',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R22 model patch missing');return r.text();}),
      fetch('urr-r22/helpers_patch.txt?v=22a',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R22 helper patch missing');return r.text();}),
      fetch('urr-r22/analyze_patch.txt?v=22a',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('R22 analyzer patch missing');return r.text();})
    ]);
    let src=pieces.join('\n');
    ...
    src=src.replace('• R20 • GRID-FIRST + DYNAMIC BELOW-TABLE • LIVE SYNC','• R22 • PADDLE ENSEMBLE + ADAPTIVE RV READER • LIVE SYNC').replace(/&v=urr-r20&cb=\$\{Date\.now\(\)\}/g,'&v=urr-r22&cb=${Date.now()}');
    ...
    (0,eval)(src);
    for(const extra of ['urr-r21/domain.js?v=22a','urr-r21/learning-hooks.js?v=22a']){
