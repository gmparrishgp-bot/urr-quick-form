...
const model=await Florence2ForConditionalGeneration.from_pretrained(model_id,{dtype:'q4',device:'cpu'});
let passes=0;
for(const c of CASES){
  const p=`/tmp/${c.name}.jpg`;await fs.writeFile(p,Buffer.from(c.data,'base64'));const image=await RawImage.read(p);
  const task='<OCR>'; const prompts=processor.construct_prompts(task); const inputs=await processor(image,prompts);
  const ids=await model.generate({...inputs,max_new_tokens:80,num_beams:1,do_sample:false});
  const decoded=processor.batch_decode(ids,{skip_special_tokens:false})[0];
  const post=processor.post_process_generation(decoded,task,image.size);
  const got=String(post?.[task]??decoded??'').trim();
  const e=wer(got,c.expected);const pass=e<=0.35;if(pass)passes++;
