...
function wer(got,expected){
  const a=norm(got).split(' '),b=norm(expected).split(' ');
...
for(const c of CASES){
  const p=`/tmp/${c.name}.jpg`;
  await fs.writeFile(p,Buffer.from(c.data,'base64'));
  const image=await RawImage.read(p);
  const messages=[{role:'user',content:[
    {type:'image'},
    {type:'text',text:'Transcribe ONLY the handwritten RV repair line in this image. Return only the handwriting, with no explanation. Preserve dollar signs, decimals, abbreviations such as DS, ODS, WBP, LP, WH, and HR. Ignore printed form lines.'}
  ]}];
  ...
  const got=(decoded?.[0]||'').trim();
  const e=wer(got,c.expected);
  const pass=e<=0.35;
  if(pass)passes++;
  console.log(JSON.stringify({name:c.name,expected:c.expected,got,wer:+e.toFixed(3),pass}));
}
console.log(`SUMMARY ${passes}/${CASES.length} pass`);
if(passes<2)process.exit(2);
