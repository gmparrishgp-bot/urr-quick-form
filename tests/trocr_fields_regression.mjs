...
async function crop(im,x0,x1,key){const sh=im.toSharp();const left=Math.max(0,Math.floor(im.width*x0)),right=Math.min(im.width,Math.ceil(im.width*x1));const top=Math.min(8,Math.floor(im.height*.08)),height=Math.max(20,im.height-top*2);const out=`/tmp/${key}.jpg`;await sh.extract({left,top,width:right-left,height}).grayscale().normalize().resize({height:160,kernel:'lanczos3'}).jpeg({quality:94}).toFile(out);return RawImage.read(out);}
const ocr=await pipeline('image-to-text','Xenova/trocr-base-handwritten',{dtype:'q8'});
let pass=0,total=0;
for(const row of rows){const s=specs[row.name];if(!s)continue;const p=`/tmp/${row.name}.jpg`;await fs.writeFile(p,Buffer.from(row.data,'base64'));const im=await RawImage.read(p);for(const [field,x0,x1,expected] of [['desc',.04,s.descEnd,s.desc],['parts',s.parts0,s.parts1,s.parts],['hours',s.hours0,s.hours1,s.hours]]){const c=await crop(im,x0,x1,`${row.name}_${field}`);const r=await ocr(c,{max_new_tokens:48});const got=(Array.isArray(r)?r[0]?.generated_text:r?.generated_text)||'';const e=wer(got,expected);const ok=e<=.34;total++;if(ok)pass++;console.log(JSON.stringify({row:row.name,field,expected,got,wer:+e.toFixed(3),pass:ok}));}}
